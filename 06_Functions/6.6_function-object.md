...
함수의 자료형은 무엇? 함수 = 객체
호출 가능한 "행동객체" 호출도 가능하지만 객채처럼 함수에 프로퍼티를 추가, 제거, 참조를 통해 전달 가능.
## ‘name’ 프로퍼티 _ 함수 이름을 가져올 수 있음.
...
객체메서드 이름은 자동 할당이 되지 않는다. 실무에서 대부분의 함수는 이름이 있으므로 위와 같은 상황은 잘 발생하지 않음.
```javascript
// 배열 안에서 함수를 생성함
let arr = [function() {}];
...
```
## ‘length’ 프로퍼티
내장 프로퍼티 length는 함수 매개변수의 개수를 반환합니다.
...
length 프로퍼티는 다른 함수 안에서 동작하는 함수의 타입을 검사(type introspection) 할 때도 종종 사용됩니다.
ex>
사용자가 답을 제출하면 ask는 핸들러 함수를 호출합니다. 이때 우리는 두 종류의 핸들러 함수를 ask에 전달할 수 있습니다.
- 인수가 없는 함수로, 사용자가 OK를 클릭했을 때만 호출됨
- 인수가 있는 함수로, 사용자가 OK를 클릭하든 Cancel을 클릭하든 호출됨

handler.length 프로퍼티를 사용하면 상황에 맞는 handler를 호출할 수 있습
...
```
인수의 종류에 따라(위 예시에선 인수의 length 프로퍼티 값에 따라) 인수를 다르게 처리하는 방식을 프로그래밍 언어에선 다형성(polymorphism) 이라고 부릅

## 커스텀 프로퍼티
함수에 자체적으로 만든 프로퍼티를 추가할 수도 있습니다.
함수 호출 횟수를 counter 프로퍼티에 저장
...
*클로저* 는 '함수 프로퍼티'로 대체할 수 있습니다. 변수의 유효범위와 클로저 챕터에서 살펴본 바 있는 counter 함수를 함수 프로퍼티를 사용해 바꿔보도록 하겠습니다.
```javascript
...
이름을 붙여도 위 함수는 여전히 함수 표현식이라는 점에 주목해야 합니다. function 뒤에 "func"이라는 이름을 붙이더라도 여전히 표현식을 할당한 형태를 유지하기 때문에 함수 선언문으로 바뀌지 않습니다.
...
함수 sayHi를 예시로 이에 대해 살펴봅시다. 함수 sayHi는 who에 값이 없는 경우, 인수 "Guest"를 받고 # 자기 자신을 호출/합니다.
...
//  그렇지만 대부분의 개발자들은 아래와 같은 코드를 작성함
let sayHi = function(who) {
  if (who) {
    alert(`Hello, ${who}`);
  } else {
...
// 이렇게 코드를 작성하면 외부 코드에 의해 sayHi가 변경될 수 있다는 문제가 생깁니다
```
